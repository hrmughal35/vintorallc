...
import { productHierarchy, getMainCategories } from '../../data/products'
...
const CategoryForm = () => {
  ...
  useEffect(() => {
    if (isEditMode && categoryId) {
      const category = productHierarchy[categoryId]
      if (category) {
        setFormData({
          name: category.name || '',
          icon: category.icon || '📦',
  ...
  const validateForm = () => {
    ...
    // Check for duplicate category name (if editing, exclude current category)
    const existingCategories = getMainCategories()
    const duplicateCategory = existingCategories.find(
      cat => cat.name.toLowerCase() === formData.name.toLowerCase().trim() && 
      (!isEditMode || cat.id !== categoryId)
    )
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              required
              placeholder="e.g., Cups, Containers, Disposable Products"
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              <strong>Note:</strong> After creating this category, you'll be able to add subcategories and products to it.
            </p>
          </div>
